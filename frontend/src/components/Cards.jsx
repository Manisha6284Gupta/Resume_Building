...
// import { Edit, Trash2, Clock, Award, TrendingUp, Zap, Check } from "lucide-react";
...
//   return (
//     <div
//       className={`group h-[200px] md:h-[300px] lg:h-[320px] flex flex-col bg-white border-2 overflow-hidden cursor-pointer transition-all duration-500 hover:scale-105 hover:shadow-lg rounded-3xl ${
//         isSelected
...
//             </div>
//           )}
//           <div className="absolute inset-0 bg-gradient-to-t from-violet-100/30 to-transparent opacity-0 group-hover:opacity-100 duration-300" />
//         </div>
//       ) : (
//         <div className="w-full h-[200px] flex items-center justify-center bg-gradient-to-br from-violet-50 via-violet-100 to-fuchsia-50">
...
export const ResumeSummaryCard = ({
  ...
  const getCompletionState = () => {
    if (completion >= 90)
      return { color: cardStyles.completionHigh, icon: <Award size={12} /> };
    if (completion >= 70)
      return { color: cardStyles.completionMedium, icon: <TrendingUp size={12} /> };
    return { color: cardStyles.completionLow, icon: <Zap size={12} /> };
  };
  ...
  const { color, icon } = getCompletionState();
  ...
  return (
    ...
    >
      {/* Completion Indicator */}
      <div className={cardStyles.completionIndicator}>
        <div className={`${cardStyles.completionDot} bg-gradient-to-r ${color}`}>
          <div className={cardStyles.completionDotInner} />
        </div>
      ...
      <div className={cardStyles.infoArea}>
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            ...
            <div className={cardStyles.dateInfo}>
              ...
              <span className="ml-2">Updated At: {formattedDate(updatedAt)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative w-full h-2 bg-gray-200 rounded-full overflow-hidden">
          <div
            className={`h-full bg-gradient-to-r ${color} rounded-full transition-all duration-700 relative`}
            style={{ width: `${completion}%` }}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent animate-pulse" />
          </div>
          <div
            className="absolute top-0 h-full w-4 bg-gradient-to-r from-transparent to-white/50 blur-sm"
            style={{ left: `${Math.max(0, completion - 2)}%` }}
          />
        </div>
  ...
  return (
    ...
    >
      {thumbnailImg ? (
        ...
        <div className="w-full h-[200px] flex flex-col items-center justify-center bg-gradient-to-br from-violet-50 via-violet-100 to-fuchsia-50">
          <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-2xl flex items-center justify-center mb-3">
            <Edit className="text-white" size={20} />
          </div>
          <span className="text-gray-700 font-bold">No Preview</span>
